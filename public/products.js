...
// Formate un outfit pour le frontend (outfit_name → name, origine → region)
function format(o) {
  return { ...o, name: o.outfit_name, region: o.origine };
}

router.get('/', async (req, res) => {
  try {
    const outfits = await prisma.outfit.findMany({ orderBy: { id: 'asc' } });
    res.json(outfits.map(format));
  } catch (error) {
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!outfit_name || !type || !brand || !origine || !price)
      return res.status(400).json({ error: 'Champs obligatoires manquants : outfit_name, type, brand, origine, price' });
    const outfit = await prisma.outfit.create({
      data: { outfit_name, type, brand, origine, price: parseFloat(price), collab: collab || false, image, description, stock: stock || 10 }
    });
    res.status(201).json(format(outfit));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
